...
const MockFirebaseAnalytics = {
    initializeFirebase: (config: any) => {
        console.warn("Firebase initialized with config:", config);
    },
    setUserId: async ({ userId }: { userId: string }) => {
        console.warn("Set user ID:", userId);
    },
    setUserProperty: async ({
        ...
        console.warn(`Set user property: ${name} = ${value}`);
    },
    logEvent: async ({
        name,
        params,
        ...
        console.warn(`Logged event: ${name} with params:`, params);
    },
};
...
export const useFirebaseAnalyticsAdapter = (): AnalyticsPort => {
    try {
        MockFirebaseAnalytics.initializeFirebase({
            ...
            storageBucket: import.meta.env
                .VITE_FIREBASE_ANALYTICS_STORAGE_BUCKET,
            messagingSenderId: import.meta.env
                .VITE_FIREBASE_ANALYTICS_MESSAGING_SENDER_ID,
            appId: import.meta.env.VITE_FIREBASE_ANALYTICS_APP_ID,
            measurementId: import.meta.env
                .VITE_FIREBASE_ANALYTICS_MEASUREMENT_ID,
        });
    } catch (e) {
        console.warn(
            "Firebase Analytics is not available in this environment",
            e
        );
    }
    ...
    return {
        ...
        async setUserProperty(
            ...
            await MockFirebaseAnalytics.setUserProperty({
                name: propertyName,
                value,
            });
        },
